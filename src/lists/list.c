...
/*   Updated: 2025/07/21 13:30:19 by mmisumi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		cmd = ft_strdup(args[0]);
// 		if (!cmd)
// 			return (free(node), free_arr(args[0]), NULL);
// 	}
// 	if (handle_files(tokens, index, fd) == HEREDOC_TERMINATED)
// 		return (free(node), free_arr(args), free(cmd), NULL);
// 	//if file_count is 0 we will not go into function
// 	// file = NULL;
// 	// if (count_redir_files(tokens, index) > 0)
// 	// {
// 	// 	file = get_file_list(file, tokens, index, file_count);
// 	// 	if (!file)
// 	// 		return (free(node), free_arr(args), free(cmd), NULL);
